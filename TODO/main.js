    ...
    function createTodoItem(obj = {}, todoList) {
        ...
        doneButton.addEventListener('click',function (event){
          ...
          if (event.target.id == "true") {
            ...
            fetchPatch(event.target.parentElement.parentElement.id,{done:false})
          }else{
            event.target.id = "true"
            fetchPatch(event.target.parentElement.parentElement.id,{done:true})
          }
        });
        ...
        deleteButton.addEventListener('click', function (event){
          if (confirm('Вы уверены?')) {
              item.remove();
              fetchDelete(event.target.parentElement.parentElement.id)
          }
        });
